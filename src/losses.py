...
class MultiTaskLoss(nn.Module):
    ...
    def __init__(self, num_tasks=4):
        ...
        # Losses
        self.loss_part = FocalLoss(alpha=0.25, gamma=3.0) # Handle imbalance
        self.loss_side = nn.BCEWithLogitsLoss()
        self.loss_price = nn.SmoothL1Loss()
        self.loss_qty = nn.MSELoss()
    ...
    def forward(self, preds, targets):
        ...
        L3 = self.loss_price(l_price_pred.squeeze()[mask], y_price[mask])

        # --- Task 4: Quantity (Regression) ---
        L4 = self.loss_qty(l_qty_pred.squeeze()[mask], y_qty[mask])
